...
import { Package, Truck, MapPin, Calendar, CreditCard, ArrowLeft, CheckCircle, RefreshCw } from "lucide-react"
...
export default async function AccountOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 1. Payment is paid
  // 2. Order is not already cancelled, returned, or exchanged
  // 3. Order has NOT been shipped OR order is delivered (can cancel after delivery)
  ...
  // Return/Exchange is allowed only if:
  // 1. Payment is paid
  // 2. Order is not cancelled, returned, or exchanged
  // 3. Order has NOT been shipped (cannot return after shipping - including after delivery)
  ...
  return (
    <>
      ...
      <MobilePageHeader title="Order Details" backHref="/account" />

      <main className="min-h-screen bg-gray-50 w-full overflow-x-hidden pt-16 lg:pt-0">
        <div className="w-full px-4 md:px-10 lg:px-16 py-8 md:py-12 pb-8 md:pb-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <CardContent className="p-6">
                    <div className="space-y-2">
                      <p className="font-bold text-lg">
                        {order.shipping_address?.fullName || order.shipping_address?.full_name || order.shipping_info?.fullName || order.shipping_info?.full_name}
                      </p>
                      ...
                      <p>{order.shipping_address?.country || order.shipping_info?.country || "India"}</p>
                        ...
                        {(order.shipping_address?.email || order.shipping_info?.email) && (
                          <p className="mt-1">
                            <span className="font-bold">Email:</span> {order.shipping_address?.email || order.shipping_info?.email}
                          </p>
                        )}
                    </div>
                  </CardContent>
            ...
            <div className="space-y-6">
              {/* Order Summary */}
              <Card className="border-2 border-black/10 sticky top-4">
                <CardHeader className="bg-black/5">
                ...
                <Card className="border-2 border-yellow-500">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      If you just paid, please refresh the page to see updated status
                    </p>
                  </CardContent>
                </Card>
              )}
              ...
              {order.payment_status === "pending" && order.payment_method === "cod" && (
                <Card className="border-2 border-blue-500">
                  <CardContent className="p-6 text-center">
                    <p className="text-sm text-muted-foreground mb-2">Cash on Delivery Order</p>
              ...
              {returnRequest && (
                <Card className="border-2 border-blue-500">
                  <CardContent className="p-6">
                    <h4 className="font-black uppercase mb-2">Return Request</h4>
                    <Badge className={`${getStatusColor(returnRequest.status)} border-2 mb-2`}>
              ...
              {exchangeRequest && (
                <Card className="border-2 border-blue-500">
                  <CardContent className="p-6">
                    <h4 className="font-black uppercase mb-2">Exchange Request</h4>
                    <Badge className={`${getStatusColor(exchangeRequest.status)} border-2 mb-2`}>
                      {exchangeRequest.status}
                    </Badge>
                    <p className="text-sm text-muted-foreground">{exchangeRequest.reason}</p>
                  </CardContent>
                </Card>
              )}
            </div>
